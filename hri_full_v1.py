...
def parse_questionnaire_input(questionnaire):
    ...
    if isinstance(questionnaire, list):
        return normalize_questionnaire_list(questionnaire)
    text = str(questionnaire or "").strip()
    if not text:
        return []
    return normalize_questionnaire_list(text)
...
работают в облаке (RouterAI и Яндекс SpeechKit). Для видео на компьютере нужен
**ffmpeg** (на Mac обычно: `brew install ffmpeg`).

**Запуск на Mac:** один раз `./scripts/install_mac_launcher.sh` — на рабочем столе
появятся **Start HR Agent** и **Stop HR Agent** (веб + бот в фоне). Логи: `run/logs/`.
...
**Создание новой вакансии** — сначала название и чат Telegram (чат должен быть
заранее в Настройках), затем генерация или импорт документов. Расшифровка разговора —
через загрузку аудио или видео (до **600 МБ**) или готовый текст.

**Документы у открытой вакансии** — блок «Создать или обновить документы»: можно
догенерировать пакет в любой момент (не только при создании). Режим **«Профиль + дополнения»**
— когда профиль уже есть, а нужно добавить запись встречи или файлы. В предпросмотре
выбираете, какие поля заменить; пакет можно сохранить в историю без применения.

**Прошлые генерации** — `data/history`. Подставить пакет: «Документы по вакансии» →
«Прошлые генерации» или подсказка при создании вакансии с похожим названием.

**Статистика** — продуктивность, сравнение периодов, ИИ-анализ; реестр «На гарантии».

**Настройки** — чаты Telegram по отделам, статус бота, Google Calendar, гарантия.

**Инструкции** (эта вкладка) — руководство по приложению, Telegram и клиентским зонам.

**Боковая панель** — ссылки на клиентские зоны (`/master`, `/client?dept=...`),
проверка баланса облачных сервисов.
    """)

    with st.expander("Настройка нового Telegram-чата", expanded=False):
        st.markdown("""
Пошагово — для новой группы с руководителем или нового отдела.

**1. Токен бота**  
В `.env` должен быть `TELEGRAM_BOT_TOKEN` (получить у [@BotFather](https://t.me/BotFather)).
После смены токена перезапустите бота.

**2. Запустите бота**  
Через **Start HR Agent** на рабочем столе или в терминале: `./venv/bin/python bot.py`.  
Без работающего бота команды `/chatid` и кнопки под карточками не отвечают.

**3. Создайте группу в Telegram**  
Обычная группа для вакансии или отдела. Добавьте в неё бота (поиск по username бота).  
Рекомендуется сделать бота **администратором** — тогда он сможет удалять служебные
сообщения при необходимости.

**4. Узнайте Chat ID**  
В группе напишите `/chatid` (или `/id`). Бот ответит числом вида `-1001234567890`.  
Скопируйте его целиком, включая минус.

**5. Сохраните чат в приложении**  
**Настройки** → **Мои чаты Telegram** → название (например, «Отдел закупок»),
Chat ID, подразделение (существующее или новое) → **Сохранить чат**.

**6. Привяжите участников**  
Каждый руководитель в группе один раз пишет `/start` — бот запомнит отдел для личных
уведомлений и клиентской зоны.

**7. Создайте вакансию**  
**Создание новой вакансии** → выберите сохранённый чат в списке. Без чата в Настройках
вакансию создать нельзя.

**8. Проверка**  
В Настройках → **Telegram-бот** — зелёный статус и тестовое сообщение в чат.  
Отправьте тестового кандидата из карточки — под сообщением должны появиться кнопки статуса.

**Частые ошибки**

- Кнопок нет — бот не запущен или карточка отправлена до запуска бота (отправьте заново).
- Два `bot.py` с одним токеном — остановите лишний процесс (`./scripts/stop_local.sh`).
- Неверный Chat ID — перепроверьте `/chatid` именно в той группе, куда шлёте кандидатов.
- `api.telegram.org` недоступен — в `.env` укажите `TELEGRAM_PROXY` (см. `.env.example`).
        """)

    with st.expander("Документы по вакансии"):
        st.markdown("""
В открытой вакансии: **Документы по вакансии** → **Создать или обновить документы**.

| Режим | Когда |
|-------|--------|
| **Профиль + дополнения** | Профиль уже в вакансии или в файле + запись/файлы с встречи |
| **Из материалов** | Аудио, видео, PDF, DOCX, текст — полный пакет с нуля |
| **Готовые документы** | Вставить уже написанные профиль, опросник, текст, ключевые слова |
| **Скорректировать существующие** | Указания HR и опционально новый файл |
| **Анкета HR** | Пошаговый опрос, как при первом создании |

После генерации — **предпросмотр**: галочки «что заменить», применение к вакансии,
скачивание Word/PDF. **Сохранить пакет в историю** — если пакет для другой должности
или черновик (не трогает текущую вакансию).

Видео до 600 МБ: нужен ffmpeg. При запуске с ярлыка Mac путь к ffmpeg подставляется
автоматически; после обновления кода перезапустите приложение.
        """)
...
можно включить «Не удалять событие из Google Calendar».

Кнопка **«Отправить в общий чат»** публикует карточку руководителю. Бот должен быть
запущен. Chat ID — в Настройках или `/chatid` в группе.

**Доп. материал в чат** — название и ссылка (запись собеседования, задание). Сообщение
уходит ответом на карточку, основная карточка обновляется списком материалов.
        """)
...
Эту же инструкцию можно отправить руководителям в группу: **Настройки → Telegram-бот →
«Отправить инструкцию в чат»** (выберите чат).
        """)
...
вынести приложение на сервер и указать `PUBLIC_APP_BASE_URL` в `.env`.
    ...
    with st.expander("Запуск бота и технические мелочи"):
        st.markdown("""
**Mac (рекомендуется):** `./scripts/install_mac_launcher.sh` → ярлыки Start/Stop HR Agent на рабочем столе.
Логи: `run/logs/`. Статус: `./scripts/status_local.sh`.

**Вручную** — бот отдельно от Streamlit:

```bash
./venv/bin/python bot.py
```

В `.env`: `TELEGRAM_BOT_TOKEN`, `TELEGRAM_HR_CONFIRM_USERNAME`, ключи SpeechKit.
Для видео — ffmpeg (`brew install ffmpeg` на Mac).

Свой Telegram user id: напишите боту в личку `/id`.
Полная настройка нового чата — expander **«Настройка нового Telegram-чата»** на вкладке Инструкции.
        """)
...
with tab_settings:
    ...
    with st.expander("📂 Мои чаты Telegram", expanded=True):
        st.caption(
            "Новый чат: создайте группу → добавьте бота → `/chatid` в группе → "
            "сохраните ID здесь. Подробно: вкладка **Инструкции** → «Настройка нового Telegram-чата»."
        )
        chats = load_chats()
        departments = load_departments()
    ...
    with st.expander("🤖 Telegram-бот", expanded=True):
        ...
        st.markdown(
            """
**Чтобы бот отвечал на команды** (`/id`, `/start`, `/chatid`) **и обрабатывал кнопки в чате**, запустите его:

- **Start HR Agent** на рабочем столе (Mac), или
- `./venv/bin/python bot.py` в терминале, или
- `./scripts/start_local.sh` (веб + бот).

Не держите два процесса `bot.py` с одним токеном.

**Chat ID группы:** бот в группе → `/chatid`. **Свой user_id:** личка с ботом → `/id`.

**Отправка кандидатов** идёт через API (бот может быть выключен), но для **кнопок и команд**
бот должен работать и быть **добавлен в группу**.
            """
        )

        test_chats = load_chats()
        if ok and test_chats:
            from telegram_bot_commands import CHAT_INSTRUCTIONS_HTML

            test_chat_names = [c["name"] for c in test_chats]
            st.markdown("##### Сообщения в чат")
            instruct_pick = st.selectbox(
                "Чат для инструкции",
                [""] + test_chat_names,
                key="settings_tg_instruct_chat",
                help="Отправит в группу краткую инструкцию по кнопкам статусов и комментариям.",
            )
            if st.button(
                "📖 Отправить инструкцию в чат",
                key="settings_tg_instruct_btn",
                disabled=not instruct_pick,
            ):
                chat = next(c for c in test_chats if c["name"] == instruct_pick)
                i_ok, i_msg, _ = send_telegram_html(chat["id"], CHAT_INSTRUCTIONS_HTML)
                if i_ok:
                    st.success(f"Инструкция отправлена в «{instruct_pick}».")
                else:
                    st.error(i_msg)
            ...
            if test_pick and st.button("📨 Отправить тестовое сообщение", key="settings_tg_test_btn"):
                ...
                if t_ok:
                    ...
                    st.error(t_msg)
        elif ok and not test_chats:
            st.info("Сначала добавьте чат в блоке «Мои чаты Telegram» выше.")
