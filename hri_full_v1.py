...
Если карточка старая — отправьте кандидата заново из приложения. На сервере
должен работать один экземпляр бота с вашим токеном, иначе сообщения могут
теряться.
        """)
...
Для доступа из интернета планируется Cloudflare Tunnel с входом по email.
        """)
