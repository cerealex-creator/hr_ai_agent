...
## 2026-06-24 — HR: убрать текст под заголовком, сайдбар

**Тип:** `fix`

**Сделано:**
- Под заголовком «HR-помогатор v2.0» убраны строки «Разработчик» и «HR-платформа…»
- «Разработчик: А.А. Крупин» — внизу левой панели
- Сайдбар по умолчанию свёрнут; кнопка сворачивания заметнее (CSS)

**Файлы:** `hri_full_v1.py`, `corporate_ui.py`

**Git:** незакоммичено

**Риски/регрессии:** нужен **перезапуск Streamlit** (`fileWatcherType = none` в `.streamlit/config.toml`)

**Следующий шаг:**
- Cmd+R в браузере после перезапуска сервера

---

## 2026-06-24 — Client zone: фокус-кандидат наверху списка

**Тип:** `fix`

**Сделано:**
- При deep link (`candidate_id`) нужный кандидат показывается **первым** в списке, а не внизу под свёрнутыми карточками
- Кнопка ссылки на видео переименована: «Запись» → «Собеседование»

**Файлы:** `client_zone.py`

**Git:** незакоммичено

**Следующий шаг:**
- Перезагрузить client zone с deep link и проверить порядок на 390px

---

## 2026-06-24 — Фаза 2: mobile UI, deep links, TG без кнопок

**Тип:** `feature`

**Сделано:**
- Client zone: mobile CSS, профиль вакансии, портфолио/HR-коммент, deep link `vacancy_id` + `candidate_id`
- Telegram: primary send без inline-кнопок, ссылка на карточку в client zone
- HR: mobile CSS в `corporate_ui.py`, ссылка на карточку в воронке кандидата
- Master zone: блок профиля вакансии

**Файлы:** `client_access.py`, `client_zone.py`, `pages/client.py`, `pages/master.py`, `telegram_client.py`, `corporate_ui.py`, `candidate_funnel.py`, `hri_full_v1.py`, `.cursor/AUDIT.md`

**Данные / конфиг:** `PUBLIC_APP_BASE_URL` — для абсолютных ссылок в TG

**Git:** незакоммичено

**Риски/регрессии:** без `PUBLIC_APP_BASE_URL` ссылка в TG может быть неполной; нужен перезапуск Streamlit

**Следующий шаг:**
- Перезапустить Streamlit и проверить deep link + mobile; фаза 3 — Timeweb
...
## 2026-06-24 — Фаза 1: токены client zone + HR-пароль

**Тип:** `feature` · `security`

**Сделано:**
- Секретные ссылки `/client?t=…` и `/master?t=…`; `?dept=` отключён.
- HR-панель: опциональный пароль `HR_APP_PASSWORD`.
- Настройки: блок копирования ссылок для чатов.

**Файлы:** `client_access.py`, `hr_auth.py`, `pages/client.py`, `pages/master.py`, `hri_full_v1.py`, `.env.example`, `data/departments.json`, `data/zone_access.json`

**Git:** незакоммичено

**Следующий шаг:**
- Перезапустить Streamlit; фаза 2 — mobile UI.

---

## 2026-06-24 — Решение: B + Timeweb, security, mobile

**Тип:** `decision`

**Сделано:**
- Зафиксировано основное направление: Client zone на Timeweb, бот на Mac+VPN.
- В `AUDIT.md` — дорожная карта фаз 0–6.

**Файлы:** `.cursor/AUDIT.md`

**Git:** незакоммичено

**Следующий шаг:**
- Фаза 1: бриф безопасности, согласование auth.

---

## 2026-06-24 — UI-аудит Streamlit (этап C)

**Тип:** `audit`

**Сделано:**
- Пройдены HR-панель и client zone (Маркетинг, 4 кандидата); mobile 390px.
- Зафиксировано 14 UI-находок; сценарий B (client zone) пригоден с доработками.
- ИИ-блок в client zone — сильное преимущество перед текущим Telegram.
...
**Следующий шаг:**
- Тест Telegram «ссылка без кнопок»; прототип deep link.

---

## 2026-06-24 — Сценарий B, аналитика, разъяснение про бота

**Тип:** `audit` · `decision`

**Сделано:**
- В `AUDIT.md`: индекс и сравнение сценариев A (Yougile) / B (mobile client zone) / C (гибрид).
- Gap-анализ client zone: нет deep link, портфолио, профиля должности, mobile CSS.
- Зафиксировано: бот не нужен для отправки в чат, но нужен для напоминаний HR (пока логика в `bot.py`).
- Требования аналитики, мульти-клиент, пристройки документов — в журнале.
...
**Следующий шаг:**
- Тест client zone на мобилке; сравнение A vs B в тестовом чате.

---

## 2026-06-24 — Yougile: ссылка в TG, состав карточки, план тестов

**Тип:** `decision` · `audit`

**Сделано:**
- В `AUDIT.md` зафиксированы: ссылка на Yougile вместо TG-кнопок; состав карточки (ИИ + медиа + HR); TG под вопросом после A/B теста.
- Gap-анализ: ИИ-оценка сейчас не уходит в Telegram — нужен новый билдер для Yougile.
- Черновик плана тестов T1–T5 в отдельной ветке `feature/yougile-export`.
...
**Следующий шаг:**
- UI-аудит HR-панели; по «ок» — прототип Yougile в тестовой ветке.

---

## 2026-06-24 — Аудит: гипотеза Yougile + локальные данные

**Тип:** `audit`

**Сделано:**
- В `AUDIT.md` зафиксирована рабочая гипотеза: локальный HR, отказ от client zone, вывод в Telegram + Yougile.
- Этап B: проанализирован `data/vacancies_db.json` — 8 вакансий, 57 кандидатов, 1.22 МБ.
- Yougile в коде нет; найдены точки переиспользования (`telegram_notify` билдеры сообщений).
- Вывод: `bot.py` не нужен для исходящих сообщений; нужен для интерактива в чате.
...
**Следующий шаг:**
- UI-обход HR-панели; уточнить: Telegram только уведомления или кнопки остаются.

---

## 2026-06-24 — Журнал аудитов и улучшений

**Тип:** `checkpoint`

**Сделано:**
- Создан `.cursor/AUDIT.md` — журнал аудитов с контекстом приложения, сводкой находок и черновиком выводов по стеку.
- Этап A (статический аудит, сессия 1): 15 находок (2🔴 / 7🟡 / 6🟢).
- Ключевые 🔴: нет авторизации в Streamlit; две разные `migrate_candidate` (`models` vs `vacancy_store`).
...
**Следующий шаг:**
- Этап B: smoke-тест запуска; этап C: UI в браузере; уточнить планируемое расширение функционала.
