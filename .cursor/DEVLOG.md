...
## 2026-09-05 — Консалтинг: волна 2 (Мегамейд, эталон, опрос)

**Тип:** `feature`

**Сделано:**
- Библиотека Мегамейд (seed) и копирование узла в эталон проекта вручную.
- Эталон: список, правка, фиксация / «не применимо».
- Опрос в приложении: полный по умолчанию, галка «добить белые пятна»; преамбула утверждается до публикации.
- Публичная ссылка `/consulting/s/[token]` и режим интервьюера; ответ «не знаю» не закрывает клетку покрытия.
- На хабе: ссылки на опрос / Мегамейд / эталон; счётчик черновиков опроса.

**Файлы:** `consulting_models.py`, `consulting_wave2.py`, `consulting.py`, `q7r8s9t0u1v2_consulting_wave2.py`, `routes/consulting.py`, `AuthGate.tsx`, `ConsultingShell.tsx`, страницы megamaid/etalon/survey/s/[token], `test_consulting_wave2.py`

**Данные / конфиг:** таблицы megamaid/etalon/process_cards/surveys/questions/responses. Миграция `q7r8s9t0u1v2` применена локально.

**Git:** ветка `feature/consulting`

**Риски/регрессии:** формы результата ещё нет — итоги и ведомость не собираем. Process cards API есть, отдельного экрана нет.

**Следующий шаг:**
- Формы результата или волна 3 после `КОНСАЛТИНГ` (только после форм).

## 2026-09-05 — Консалтинг: волна 1 без форм

**Тип:** `feature`

**Сделано:**
- Встречи (уровень, дата, расшифровка, папка), противоречия (в т.ч. авто из «оспорено»).
- Белые пятна по каркасу в коде (скилл): клетка закрывается только годным следом, не словом в поиске.
- Публичный Яндекс: файл разбирается, папка даёт список, видео не качаем.
- Витрина-каркас `/consulting/p/[token]`: комментарий и «Утверждаю». Гипотезы и черновики не публикуются. Итог по формам не собираем.
- Уверенность строки — по независимости следов, не по тексту ИИ.

**Файлы:** `consulting_models.py`, `consulting.py`, `consulting_coverage.py`, `consulting_extract.py`, `p6q7r8s9t0u1_consulting_wave1.py`, `routes/consulting.py`, `AuthGate.tsx`, `ConsultingShell.tsx`, страницы meetings/contradictions/coverage/p/[token]

**Данные / конфиг:** таблицы `consulting_meetings`, `consulting_contradictions`, `consulting_comments`; поля разбора у источников. Миграция применена локально.

**Git:** ветка `feature/consulting`

**Риски/регрессии:** формы результата ещё не загружаются. Локальный API после перезапуска лучше поднимать `v2/scripts/start_local.sh`.

**Следующий шаг:**
- Волна 2 (`КОНСАЛТИНГ`) или подгрузка форм, когда будут готовы.

## 2026-09-05 — Консалтинг: подгрузка форм заказчика в поиск

**Тип:** `decision`

**Сделано:**
- В плане: утверждённые формы можно загрузить файлом или вставить текстом из чата заказчика (он утверждает выдачу своей ChatGPT).
- Формы — третье пространство поиска для ИИ, не доказательства о компании и не скиллы.
- После загрузки человек подтверждает «это форма». Пример заполнения из чата — не факт о ГК.
- Список из 10 названий ТЗ — охват; если принёс свои формы, работаем по ним.
- Затвор: итоги не собираем, пока формы не загружены и не подтверждены.

**Файлы:** `IMPLEMENTATION_PLAN_CONSULTING.md`, `BACKLOG.md`, план чата «Блок Консалтинг»

**Git:** ветка `feature/consulting`

**Следующий шаг:**
- Подгрузить утверждённые формы в проект, затем волна 1 (`КОНСАЛТИНГ`).

## 2026-09-05 — Консалтинг: сначала формы результата

**Тип:** `decision`

**Сделано:**
- В плане зафиксировано: формы документов на итоги утверждаются до работы и до кода волн анализа.
- Сбор, вопросы и разбор идут от пустых клеток формы. ИИ пишет в клетку, форму не меняет.
- Итог = заполненные формы + витрина из тех же полей. Пустая клетка — «нет данных».
- Папка 09 (шаблоны заказчику) и формы нашей сдачи — разные наборы.
- Затвор: витрина с итогами, ведомость и снимок не пишем, пока формы не утверждены.
...
**Следующий шаг:**
- Утвердить 10 форм результата, затем волна 1 (`КОНСАЛТИНГ`).

## 2026-09-05 — Консалтинг: волна 0

**Тип:** `feature`

**Сделано:**
- Отдельный блок `/consulting` (не СУП): хаб, паспорт, папки 00–09, план, реестр.
- Доступ только у владельца платформы; в демо карточки нет, прямая ссылка возвращает на главную.
- ИИ не ставит «подтверждено». «Оспорено заказчиком» нельзя откатить в рекомендацию ИИ.
- Ссылка: название + цитата; закрытый URL без цитаты — отказ, публичный Яндекс — можно без цитаты.
- Паспорт: УК + 6 дирекций + 5 БЕ, точки 3/10/15/20 дней. Миграция `o5p6q7r8s9t0` применена локально.

**Файлы:** `consulting_models.py`, `consulting.py`, `consulting_folders.py`, `routes/consulting.py`, `o5p6q7r8s9t0_consulting_wave0.py`, `HomeHubCards.tsx`, `AuthGate.tsx`, `ConsultingShell.tsx`, `frontend/app/consulting/**`, `IMPLEMENTATION_PLAN_CONSULTING.md`, `BACKLOG.md`

**Данные / конфиг:** новые таблицы `consulting_*`; в локальной БД создан один проект при проверке API (2 тестовых материала, 1 человек)

**Git:** ветка `feature/consulting`

**Риски/регрессии:** загрузка настоящего файла ещё не качается — пока название + цитата. Волны 1–4 не начинали.

**Следующий шаг:**
- Волна 1 по кодовому слову `КОНСАЛТИНГ`: база знаний, встречи, витрина, белые пятна, разбор Яндекса.
...
**Сделано:**
- Создана ветка `docs/plans-kaskad-yakor-assistent-efir` от `08f15b7` с документами планов (без кода макетов/демо).
- Запушено в origin: commit `79061f4`.
- В ветке: `IMPLEMENTATION_PLAN_KASKAD|YAKOR|ASSISTENT|EFIR`, обновления `BACKLOG.md` и `QUEUE.md`; ЯКОРЬ и runbook уже были в базе `08f15b7`.

**Файлы:** `v2/docs/IMPLEMENTATION_PLAN_*.md`, `v2/docs/BACKLOG.md`, `v2/docs/QUEUE.md`, `.cursor/runbooks/yakor-person-race.md`

**Git:** branch `docs/plans-kaskad-yakor-assistent-efir`, push origin

**Следующий шаг:**
- При реализации по кодовому слову — cherry-pick или merge только нужного плана из этой ветки.

---

## 2026-08-18 — Макеты: плюсы/минусы ИИ за кнопкой

**Тип:** `fix`

**Сделано:**
- В `/m/…` плюсы и минусы ИИ скрыты.
- Кнопка «Коммент от ИИ-ассистента» стоит ниже «Посмотреть резюме» и выше «Можно рассмотреть» / «Отказ».

**Файлы:** `m/[token]/page.tsx`, `globals.css`

**Git:** незакоммичено

---

## 2026-08-18 — Макеты: решение заказчика в личку HR

**Тип:** `fix`

**Сделано:**
- При «Можно рассмотреть» / «Отказ» в зоне макетов одно сообщение уходит в личку HR (`TELEGRAM_HR_USER_ID`).
- В чат вакансии заказчика это решение больше не пишется.

**Файлы:** `resume_preview.py`

**Git:** незакоммичено

**Deploy:** rsync backend, sidecar `api`, health OK. `TELEGRAM_HR_USER_ID` задан.

**Следующий шаг:**
- Нажать решение в `/m/…` — одно сообщение должно прийти в личку бота.

---

## 2026-08-18 — Макеты: сворачиваемое добавление резюме

**Тип:** `fix`

**Сделано:**
- Блок «Ссылки на PDF без контактов» по умолчанию скрыт.
- Кнопки «Добавить резюме» / «Свернуть» раскрывают поле и «Загрузить в макеты».
- После загрузки блок снова сворачивается.

**Файлы:** `ResumePreviewPanel.tsx`, `globals.css`

**Git:** незакоммичено

**Deploy:** rsync frontend, sidecar `web`

---

## 2026-08-18 — Макеты: комментарий HR для заказчика

**Тип:** `feature`

**Сделано:**
- Отдельное поле `resume_preview_hr_comment` — только для зоны макетов, не смешивается с `hr_comment`.
- HR редактирует во вкладке «Макеты резюме» (сохранение при уходе из поля).
- Заказчик видит блок «Комментарий рекрутера» на `/m/…` между оценкой ИИ и кнопкой резюме.

**Файлы:** `resume_preview.py`, `jsonb_defaults.py`, `schemas.py`, `vacancies.py`, `ResumePreviewPanel.tsx`, `m/[token]/page.tsx`, `globals.css`

**Git:** незакоммичено

**Deploy:** rsync backend + frontend, sidecar `api` / `web`, health OK

**Следующий шаг:**
- Во вкладке «Макеты резюме» заполнить комментарий и открыть `/m/…` — блок «Комментарий рекрутера».

---

## 2026-08-18 — Макеты: короче текст в Telegram

**Тип:** `fix`

**Сделано:**
- В сообщении с макетами: «Просьба оценить возможность рассмотрения».
...
**Сделано:**
- На Timeweb DNS отдаёт `149.154.166.110`, до него из контейнера нет маршрута; рабочий Bot API — `149.154.167.220`.
- `telegram_provider.py`: запасные IPv4, подключение с SNI к `api.telegram.org`, опции `TELEGRAM_PROXY` / `TELEGRAM_API_IPV4`.

**Файлы:** `telegram_provider.py`, `config.py`

**Git:** незакоммичено

**Риски/регрессии:**
- Если Telegram сменит рабочий IP — обновить fallback или задать `TELEGRAM_API_IPV4` в `.env.sidecar`.

**Следующий шаг:**
- Повторить «Отправить в Telegram» во вкладке макетов.

---

## 2026-08-18 — Промпт оценки резюме: плюсы/минусы по профилю

**Тип:** `fix`

**Сделано:**
- В `FUNNEL_EVAL_SYSTEM` плюсы — покрытие важных и второстепенных требований профиля.
- Минусы — в том числе отсутствие этих требований в резюме, без общих фраз.

**Файлы:** `candidate_resume_eval.py`

**Git:** незакоммичено

**Deploy:** api+worker на hr-toolbox.ru; переоценка 2 макетов в очереди.

**Следующий шаг:**
- Проверить плюсы/минусы в зоне `/m/…` после завершения jobs.

---

## 2026-08-18 — Макеты: текст сообщения в Telegram

**Тип:** `fix`

**Сделано:**
- В сообщении с ссылкой на макеты добавлена фраза: «Нужно ваше мнение. Прошу оценить возможность рассмотрения этих кандидатов».

**Файлы:** `resume_preview.py`

**Git:** незакоммичено

**Deploy:** rsync backend, sidecar `api`, health OK

**Следующий шаг:**
- Отправить макеты в Telegram — в начале сообщения новая фраза.
...
**Сделано:**
- Сообщение в чат уходило, затем API падал: `NameError: _now_iso`.
- Вернул `_now_iso` — её случайно убрали при переименовании макетов.

**Файлы:** `resume_preview.py`

**Git:** незакоммичено

**Deploy:** rsync backend, sidecar `api`, health 200 на https://hr-toolbox.ru/api/v1/health

**Следующий шаг:**
- Снова нажать «Отправить в Telegram» — сообщение уйдёт без HTTP 500.

---

## 2026-08-18 — Макеты: «Кандидат на рассмотрение»

**Тип:** `fix`

**Сделано:**
- В зоне макетов вместо «Новый кандидат» показывается «Кандидат на рассмотрение».
- Новые макеты создаются с этим именем. Обычная воронка не менялась.

**Файлы:** `resume_preview.py`, `candidate_resume_eval.py`

**Git:** незакоммичено

**Deploy:** rsync backend, sidecar `api` / `worker`, health 200 на https://hr-toolbox.ru/api/v1/health

**Следующий шаг:**
- Обновить вкладку «Макеты резюме» и ссылку `/m/…`: вместо «Новый кандидат» — «Кандидат на рассмотрение».

---

## 2026-08-18 — Макеты: плюсы зелёные, минусы красные

**Тип:** `fix`

**Сделано:**
- В публичной карточке `/m/…` убраны подписи «Плюсы» / «Минусы».
- Плюсы: зелёный текст и маркеры (`--success`).
- Минусы: красный текст и маркеры (`--danger`).

**Файлы:** `m/[token]/page.tsx`, `globals.css`

**Git:** незакоммичено

**Deploy:** rsync frontend, sidecar `web`, health 200 на https://hr-toolbox.ru/api/v1/health

**Следующий шаг:**
- Обновить страницу макетов: плюсы зелёные, минусы красные, без подписей.

---

## 2026-08-18 — Макеты: минусы ИИ в лёгкой оценке

**Тип:** `fix`

**Сделано:**
- Лёгкая оценка и раньше писала `weaknesses` в карточку; теперь минусы отдаются в зону макетов `/m/…` и считаются во вкладке HR.
- В публичной карточке макета плюсы и минусы показываются отдельными списками.
- В промпт оценки добавлено требование: 2–4 конкретных плюса и минуса.

**Файлы:** `resume_preview.py`, `candidate_resume_eval.py`, `m/[token]/page.tsx`, `ResumePreviewPanel.tsx`, `globals.css`

**Git:** незакоммичено

**Deploy:** rsync backend + frontend, sidecar `api` / `worker` / `web`, health 200 на https://hr-toolbox.ru/api/v1/health

**Следующий шаг:**
- Для уже загруженных макетов минусы появятся, если ИИ их уже записал; иначе — повторить лёгкую оценку.

---

## 2026-08-18 — Макеты: фото и галочка «в зоне заказчика»

**Тип:** `fix`

**Сделано:**
- Во вкладке «Макеты резюме» у каждой строки есть фото (как в списке вакансии).
- Галочка «В зоне заказчика» показывает или скрывает макет на `/m/…`, не убирая его из вкладки.
- Кнопка «Убрать» по-прежнему выкидывает карточку из макетов обратно в основную воронку.
- Старые макеты без флага остаются видимыми заказчику.

**Файлы:** `resume_preview.py`, `schemas.py`, `jsonb_defaults.py`, `candidate_write.py`, `candidates.py`, `vacancies.py`, `ResumePreviewPanel.tsx`, `globals.css`

**Git:** незакоммичено

**Следующий шаг:**
- Выложить backend + frontend и проверить фото и галочку во вкладке макетов.

---

## 2026-08-18 — Макеты не в основном списке вакансии

**Тип:** `deploy`

**Сделано:**
- Кандидаты с флагом `resume_preview_included` скрыты из вкладки «Кандидаты» вакансии, общего списка и счётчика вакансии.
- Они остаются списком во вкладке «Макеты резюме» (счётчик на вкладке).
- Из «Требуют внимания» макеты тоже убраны.
- Вместе ушёл порядок этапов: «не отвечает» сразу перед отказами.
- Выложено на hr-toolbox.ru: rsync backend + frontend, пересборка sidecar `api` / `web`.
- Health OK после короткого старта API.

**Файлы:** `resume_preview.py`, `vacancies.py`, `common.py`, `candidate_query.py`, `stats_service.py`, `vacancies/[id]/page.tsx`, `ResumePreviewPanel.tsx`

**Git:** незакоммичено

**Следующий шаг:**
- Обновить страницу вакансии: во вкладке «Кандидаты» без «Новый кандидат»-макетов; они во вкладке «Макеты резюме».

---

## 2026-08-18 — Вакансия: «не отвечает» ниже первичного контакта

**Тип:** `fix`

**Сделано:**
- В списке кандидатов вакансии этап `no_response_3d` («Кандидат не отвечает более 3 дней») стоит сразу перед блоком отказов.
- «Первичный контакт» теперь выше, чем «не отвечает».
- Названия статусов и общая воронка не менялись.

**Файлы:** `app/api/v1/routes/vacancies.py`

**Git:** незакоммичено

**Следующий шаг:**
- Выложить backend, открыть вакансию и проверить порядок списка.

---

## 2026-08-18 — Макеты резюме: лёгкая оценка без опросника

**Тип:** `deploy`

**Сделано:**
- При загрузке ссылок/файлов в макеты (`for_resume_preview`) задачи `candidate_evaluate_resume` ставятся с `skip_questionnaire=true`.
- Остаются извлечение полей из резюме и оценка с плюсами ИИ; генерация опросника не запускается (минус 2 тяжёлых вызова RouterAI).
- Обновлены сообщения в API, панели макетов и подпись прогресса в worker.
- Выложено на hr-toolbox.ru: rsync backend + frontend, пересборка sidecar `api` / `worker` / `web`.
- Health OK: `https://hr-toolbox.ru/api/v1/health` → 200.

**Файлы:** `jobs.py`, `vacancies.py`, `candidate_resume_eval.py`, `tasks.py`, `ResumePreviewPanel.tsx`

**Git:** незакоммичено

**Следующий шаг:**
- Загрузить 2 ссылки в макеты и проверить, что задачи «Лёгкая оценка для макета…» завершаются без таймаута.

---

## 2026-08-18 — Вакансия: макеты резюме вынесены в отдельную вкладку

**Тип:** `deploy`

**Сделано:**
- Блок `Макеты резюме для заказчика` убран из вкладки `Кандидаты`.
- В карточке вакансии добавлена отдельная вкладка `Макеты резюме` сразу правее вкладки `Настройки`.
- Вкладка показывается только там, где у пользователя раньше был доступ к блоку макетов.
- Прямой переход в `?section=resume-preview` без прав мягко возвращается на `Кандидатов`.
- Локальная сборка `v2/frontend` прошла успешно.
- Выложено на hr-toolbox.ru: rsync frontend, пересборка sidecar `web`.
- Health OK: `https://hr-toolbox.ru/api/v1/health` → 200.

**Файлы:** `app/vacancies/[id]/page.tsx`

**Git:** незакоммичено

**Следующий шаг:**
- Проверить в вакансии порядок вкладок и открытие новой вкладки `Макеты резюме` на проде.

---

## 2026-08-18 — Переименование клиентской зоны в интерфейсе

**Тип:** `deploy`

**Сделано:**
- Во фронтенде пользовательское название `Клиентская зона` заменено на `Зона заказчика вакансии`.
- Обновлены меню, заголовок раздела, публичные страницы `/c/...` и блок в настройках компании.
- URL, API и внутренние пути `client-zone` не менялись.
- Выложено на hr-toolbox.ru: rsync frontend, пересборка sidecar `web`.
- Health OK: `https://hr-toolbox.ru/api/v1/health` → 200.

**Файлы:** `RecruitingShell.tsx`, `AppShell.tsx`, `client-zone/page.tsx`, `c/[token]/page.tsx`, `c/[token]/[candidateId]/page.tsx`, `CompanyEditor.tsx`

**Git:** незакоммичено

**Следующий шаг:**
- Проверить в интерфейсе новое название в меню, в настройках компании и на страницах заказчика.

---

## 2026-08-18 — Демо: клик «Поиск сотрудников» с главной

**Тип:** `deploy`

**Сделано:**
- Главная `/` больше не сбрасывает сессию: проверка входа идёт и на хабе.
- Клик по «Поиск сотрудников» ведёт на рабочий стол, а не крутит вход → обратно на главную.
- Если сессия уже есть, страница входа уважает `?next=`.
- Middleware при редиректе на вход тоже передаёт `next`.
- Выложено на hr-toolbox.ru: rsync frontend, пересборка sidecar `web`.
- Health OK: `https://hr-toolbox.ru/api/v1/health` → 200.

**Файлы:** `AuthGate.tsx`, `middleware.ts`

**Git:** незакоммичено

**Следующий шаг:**
- В демо: главная → «Поиск сотрудников» → рабочий стол.

---

## 2026-08-17 — Главная: короткие описания плашек

**Тип:** `deploy`

**Сделано:**
- Тексты четырёх заглушек на `/` заменены на короткие блоки со списками.
- Выложено на hr-toolbox.ru: rsync frontend, пересборка sidecar `web`.
...
**Следующий шаг:**
- Открыть https://hr-toolbox.ru — новые описания плашек.

---

## 2026-08-17 — Главная: тексты плашек-заглушек

**Тип:** `deploy`

**Сделано:**
- Обновлены описания четырёх модулей-заглушек на `/`.
- Плашка «Управление персоналом» переименована в «Управление и работа с персоналом».
- Метка «скоро» заменена на «В разработке».
- Демо после кнопки без регистрации тоже открывает эту главную.
- Выложено на hr-toolbox.ru: rsync frontend, пересборка sidecar `web`.

**Файлы:** `app/page.tsx`, `globals.css`, `app/login/page.tsx`

**Git:** незакоммичено

**Следующий шаг:**
- Открыть https://hr-toolbox.ru — новые тексты и «В разработке».

---

## 2026-08-17 — Демо: вход на главную с плашками

**Тип:** `fix`

**Сделано:**
- «Посмотреть демо без регистрации» открывает `/` (хаб с плашками), а не `/dashboard`.
...
**Следующий шаг:**
- Выложить на прод, если нужно сразу на hr-toolbox.ru.

---

## 2026-08-17 — Демо: одинаковые телефоны

**Тип:** `deploy`

**Сделано:**
- Телефоны вымышленных кандидатов в демо: `+7 000 000 00 00` (`DEMO_CONTENT_VERSION = 6`).
- У Беловой телефон по-прежнему пустой (сценарий «нет телефона в резюме»).

**Файлы:** `demo_showcase.py`

**Git:** незакоммичено

**Следующий шаг:**
- Открыть демо-вакансию — у кандидатов один и тот же номер.

---

## 2026-08-17 — Макеты резюме только у администратора

**Тип:** `deploy`

**Сделано:**
- Блок «Макеты резюме для заказчика» на вакансии виден только `platform_owner`.
- У рекрутера скрыты галочки макета при добавлении кандидата и поля макета в карточке.
- API `/vacancies/…/resume-preview*` требует администратора; `for_resume_preview` у рекрутера игнорируется.
- Выложено на hr-toolbox.ru: rsync backend + frontend, пересборка sidecar `api` / `web`.

**Файлы:** `ResumePreviewPanel.tsx`, `AddCandidateForm.tsx`, `CandidateEditor.tsx`, `client-zone/page.tsx`, `vacancies.py`, `candidates.py`

**Git:** незакоммичено

**Следующий шаг:**
- Админом открыть вакансию — блок макетов есть; в демо и у рекрутера — нет.

---

## 2026-08-17 — Прод-демо: изоляция и дыры

**Тип:** `deploy`

**Сделано:**
- Посев демо больше не копирует документы и опросники с боевой org `default` — только вымышленный контент (`DEMO_CONTENT_VERSION = 5`).
- Демо-GET не отдаёт Я.Диск (статус, inbox, пути) и test-chat (Telegram ID).
- GET вакансии/HH больше не пишут в базу в демо (общий демо-мир не портят).
- Сид демо при старте API и на `APP_ENV=production`.
- Лимит `POST /auth/demo`: 30 запросов с IP в час, чистка старых refresh-токенов.
- Выложено на hr-toolbox.ru: rsync backend, пересборка sidecar `api` / `worker`.
- Health OK. На проде org `demo-showcase`, версия контента **5**, 4 вакансии.

**Файлы:** `demo.py`, `demo_showcase.py`, `main.py`, `tenancy.py`, `auth.py`, `integrations.py`, `settings.py`, `vacancies.py`, `hh.py`

**Git:** незакоммичено

**Следующий шаг:**
- На https://hr-toolbox.ru нажать «Посмотреть демо без регистрации» — Аврора без боевых данных.

---

## 2026-08-17 — Документы: только читаемый вид

**Тип:** `deploy`

**Сделано:**
- Убран переключатель «Читаемый / JSON» в блоках документов.
- Документы всегда показываются в читаемом формате (вакансия и история).
- Выложено на hr-toolbox.ru: rsync frontend, пересборка sidecar `web`.
- Health OK: `https://hr-toolbox.ru/api/v1/health` → 200.

**Файлы:** `DocumentBlock.tsx`, `globals.css`

**Git:** незакоммичено

**Следующий шаг:**
- Открыть документы вакансии на проде — без кнопок Читаемый/JSON.

---

## 2026-08-17 — Deploy sidecar: документы внизу, диск, демо-изоляция

**Тип:** `deploy`

**Сделано:**
- Rsync `v2/backend` + `v2/frontend` на Timeweb `/opt/hr_ai_agent/v2`.
- Пересборка sidecar `api` / `worker` / `web`.
- Health OK: `https://hr-toolbox.ru/api/v1/health` → 200.
- На прод ушли: пересборка документов внизу, оценка/фото с Я.Диска, демо без админских гарантий и без «Ключевых слов».

**Git:** незакоммичено

**Следующий шаг:**
- На https://hr-toolbox.ru открыть вакансию с документами — блоки пересборки внизу.

---

## 2026-08-17 — Подпись «Конспект собеседования»

**Тип:** `fix`

**Сделано:**
- Пользовательские подписи «Выжимка собеседования» заменены на «Конспект собеседования»: Telegram, Bitrix, публичная `/i/…`, карточка HR, подсказки.
- Старые записи DEVLOG и промпт модели не трогал.

- Выложено на hr-toolbox.ru (`api` + `web` + `worker`).

**Файлы:** `card_html.py`, `bitrix/outbound.py`, `interview_digest.py`, `tasks.py`, `QuestionnairePanel.tsx`, `CandidateEditor.tsx`, `app/i/[token]/page.tsx`, `client-zone/page.tsx`, `ClientZoneLink.tsx`

**Git:** незакоммичено

**Следующий шаг:**
- Открыть карточку кандидата и публичную `/i/…` — везде «Конспект собеседования».

---

## 2026-08-17 — Зона макетов резюме без контактов

**Тип:** `feature`

**Сделано:**
- Отдельная публичная витрина `/m/{token}`: карточки PDF без контактов, фото, плюсы ИИ, кнопки «Посмотреть резюме» / «Можно рассмотреть» / «Отказ».
- Токен живёт на вакансии (`payload.resume_preview_token`), обычная зона `/c/` не затронута.
- HR: блок «Макеты резюме» на вакансии — ссылки Я.Диска, готовность, копия ссылки, отправка в Telegram.
- В карточке кандидата поле «Макет PDF без контактов» и флажок попадания в витрину.
- При добавлении ссылками/файлом можно отметить «это макеты»; оценка ИИ идёт в фоне (тост задач).

**Файлы:** `resume_preview.py`, `routes/resume_preview.py`, `vacancies.py`, `candidate_resume_eval.py`, `candidate_write.py`, `jsonb_defaults.py`, `keyboards.py`, `router.py`, `ResumePreviewPanel.tsx`, `app/m/[token]/page.tsx`, `CandidateEditor.tsx`, `AddCandidateForm.tsx`, `vacancies/[id]/page.tsx`, `globals.css`, `middleware.ts`, `AuthGate.tsx`

**Git:** незакоммичено

**Следующий шаг:**
- Проверить: загрузить 2 PDF-ссылки Диска → дождаться оценки → отправить в чат → открыть `/m/…` заказчиком.

---

## 2026-08-17 — Документы: пересборка внизу после заполнения

**Тип:** `fix`

**Сделано:**
- Если профиль / текст / опросник уже есть, блоки «Собрать документы по вопросам» и «Из записи / материалов встречи» стоят под списком документов.
- Пока документов нет — блоки остаются сверху, как стартовая точка.

**Файлы:** `DocumentsEditor.tsx`, `DocumentsFromBrief.tsx`

**Git:** незакоммичено. На прод не выкладывал.

**Следующий шаг:**
- Открыть вакансию с готовыми документами → вкладка «Документы»: два блока внизу.

---

## 2026-08-17 — Демо: скрыть «Ключевые слова» в документах вакансии

**Тип:** `fix`

**Сделано:**
- В демо у документов вакансии больше нет раздела «Ключевые слова»: редактор, просмотр, шапка («N док. · …»), история снимков.
- Формы «собрать из брифа / материалов» в демо не показываются (там тоже были ключевые слова).

**Файлы:** `DocumentsEditor.tsx`, `documents_preview.py`, `common.py`, `vacancies.py`, `stats_history.py`, `demo_showcase.py`

**Git:** незакоммичено

**Следующий шаг:**
- В демо открыть вакансию → Документы — блока «Ключевые слова» быть не должно.

---

## 2026-08-17 — Демо: утечка админских гарантий в аналитике

**Тип:** `fix`

**Сделано:**
- Реестр гарантий больше не читает все вакансии: только своя организация. В демо больше нет Ешкиной / «Пульс Групп».
- В демо-посеве у Смирнова своя вымышленная гарантия (реестр не пустой).
- Демо не видит админский импорт (`source_dir`), Telegram-бота, пути календаря, Bitrix/Диск из `/settings/app`.
- Каналы без `client_id` больше не светятся всем организациям.

**Файлы:** `warranty.py`, `stats_history.py`, `messaging.py`, `integrations.py`, `settings.py`, `demo_showcase.py`, `app/settings/page.tsx`

**Данные / конфиг:** локальная Postgres, org `demo-showcase` версия контента 4.

**Git:** незакоммичено

**Следующий шаг:**
- В демо обновить `/stats` — в реестре должен быть Смирнов, не админские фамилии.

---

## 2026-08-17 — Настройки: падение useUiPrefs

**Тип:** `fix`

**Сделано:**
- На `/settings` вызывался `useUiPrefs` без импорта — страница падала (`useUiPrefs is not defined`), в том числе в демо.

**Файлы:** `v2/frontend/app/settings/page.tsx`

**Git:** незакоммичено

**Следующий шаг:**
- Обновить `/settings` локально.

---

## 2026-08-17 — Диск: пустая карточка без оценки и фото

**Тип:** `fix`

**Сделано:**
- На проде заполнена карточка Чирковой `222d2b82` (фото + поля + оценка 3 + опросник) — пользователь ждал на ней, пока оценка ушла на дубль `cc25df4c`.
- Синхронизация Я.Диска после создания/пустой карточки ставит `candidate_evaluate_resume` в очередь (HTTP sync и worker).
- Скачивание резюме с `disk.yandex.ru` / `yadi.sk` / `yadisk:` идёт через прямую ссылку API, а не HTML-страницу — иначе фото не извлекалось.
- Повторная ссылка на того же человека дополняет пустую карточку, а не создаёт вторую. После «добавить ссылками» UI открывает карточку.
- Дубль `cc25df4c` на вакансии #15 оставлен — можно удалить вручную.

**Файлы:** `pdf_extract.py`, `candidate_photo.py`, `yandex_disk_sync.py`, `candidate_resume_eval.py`, `jobs.py`, `vacancies.py`, `tasks.py`, `AddCandidateForm.tsx`, `YandexDiskPanel.tsx`

**Данные / конфиг:** прод, кандидат `222d2b82-e5a1-499c-8113-bc455166cdc8` (payload скопирован с оценённого дубля, `resume_link` диска сохранён).

**Git:** незакоммичено. На прод код ещё не выложен — без выкладки новый заход с Диска снова даст пустую карточку.

**Следующий шаг:**
- Обновить карточку Чирковой в браузере. Выложить на прод, когда скажете.

---

## 2026-08-17 — Демо: только просмотр + живые данные

**Тип:** `fix`

**Сделано:**
- Гость больше не видит «Сохранить / редактировать / удалить / отправить / ИИ» в карточке кандидата и вакансии. Word-оффер скачивается без записи в базу.
- API по-прежнему режет POST/PUT/PATCH/DELETE у демо-сессии (кроме входа).
- Демо-данные пересеяны: вакансия «Менеджер маркетплейсов» без Lamoda, без дублей. Документы и опросники скопированы из админского пространства с заменой брендов (YourBox / Lamoda / «Рука на пульсе»).
- У Кузнецова заполнен оффер (обязанности, вилка, руководитель, адрес) и логотип Авроры. Скачивание .docx работает.

**Файлы:** `demo_showcase.py`, `CandidateEditor.tsx`, `CandidateOfferPanel.tsx`, `QuestionnairePanel.tsx`, `DocumentsEditor.tsx`, `VacancyTitleEditor.tsx`, `VacancySettingsPanel.tsx`, `VacancyCloseButton.tsx`, `AddCandidateForm.tsx`, `CreateVacancyForm.tsx`, `HhSearchPanel.tsx`

**Данные / конфиг:** локальная Postgres `hr_v2`, org `demo-showcase` (версия контента 3). Админские карточки не менялись.

**Git:** незакоммичено

**Следующий шаг:**
- Обновить страницу, войти в демо, открыть вакансию «Менеджер маркетплейсов», карточку Кузнецова → «Сделать оффер» → скачать Word.

---

## 2026-08-17 — Демо без регистрации (локально)

**Тип:** `feature`

**Сделано:**
- Кнопка на `/login`: «Посмотреть демо без регистрации» → `POST /api/v1/auth/demo`, роль `hr_recruiter`, сессия 4 часа, флаг `is_demo`.
- Org `demo-showcase` («Аврора Ритейл», 3 подразделения, 4 вакансии, 12 кандидатов с оценкой / опросником / конспектом).
- Read-only: любые POST/PUT/PATCH/DELETE кроме login/demo/refresh/logout → 403. Клиентская зона без решений + баннер.
- Настройки: только плашки снаружи; внутри — заглушка. Баннер: «Это демо-режим. Для полного доступа напишите: mailto@alexkrupin.ru».
- Посев локально: `python -m app.scripts.ensure_demo_showcase` (также при старте API, если не production).

**Файлы:** `demo.py`, `demo_showcase.py`, `auth.py`, `main.py`, `client_zone.py`, login/settings/AuthGate/DemoBanner, `c/[token]/*`

**Git:** незакоммичено, **на prod не выкладывалось**

**Следующий шаг:**
- Локально: вход → демо → рабочий стол / вакансии / карточка / `/c/demo-avrora-zone`. Потом тексты под ваш пилот.
...
**Сделано:**
- На prod удалены лишние 6 users (в т.ч. dialex307@gmail.com, org1@t.test, …).
- Оставлен `owner@hr.local` (пароль без изменений).
- Создан тестовый `pilot@demo.ru` / `password123`, org Demo Sandbox, роль hr_recruiter.

**Git:** не трогали

**Следующий шаг:**
- При необходимости сменить пароль pilot@demo.ru.
...
**Сделано:**
- При закрытии вакансии активные кандидаты (кроме hire/offer/уже отказанных) → `rejected_hr` с комментарием «Отказ в связи с закрытием вакансии», `status_updated_at` = момент закрытия.
- Сохраняется `vacancy_close_snapshot` для отката; при reopen ≤7 дней — восстановление прежнего этапа.
- Best-effort refresh Telegram-карточки и Bitrix sync.
- `attention_reason` пропускает все `REJECT_STAGES`.
- `reconcile_closed_vacancy_candidates` — догон для уже закрытых вакансий; прогнан на проде.

**Файлы:** `vacancy_write.py`, `candidate_query.py`, `candidate_write.py`, `labels.ts`

**Deploy:** sidecar rebuild, backfill на проде (десятки кандидатов переведены)

**Следующий шаг:**
- Проверить архив закрытых вакансий — кандидаты не должны быть в «требуют внимания».
...
**Сделано:**
- `build_candidate_card_html_minimal` — после статуса показывает комментарий клиента (как полная карточка 15.08): «Комментарий к статусу» / «Комментарий», дата встречи для ready.
- `refresh_card_message` (minimal) передаёт `client_comment` и поля встречи из `candidate_view_dict`.
- Общий хелпер `_append_client_comment_lines` для full и minimal карточек.

**Файлы:** `card_html.py`, `inbound.py`

**Git:** незакоммичено

**Deploy:** rsync + sidecar rebuild (`api`/`worker`/`web`), health 200 на https://hr-toolbox.ru/api/v1/health

**Следующий шаг:**
- Deploy sidecar → перерисовать карточку Бабич (decide или refresh из HR).

---

## 2026-08-17 — Deploy: minimal-карточка + client zone + Bitrix sync

**Тип:** `deploy`

**Сделано:**
- Rsync на Timeweb `201.34.137.208`, пересборка sidecar `api` / `worker` / `web`.
- На проде: комментарий в minimal Telegram-карточке, Bitrix sync после decide в client zone, redirect «Решение сохранено».
- Health OK: `https://hr-toolbox.ru/api/v1/health` → 200.
- Догон Бабич в Telegram: `editMessageText` — таймаут до `api.telegram.org` с сервера (карточка пока без комментария в чате).

**Git:** незакоммичено

**Следующий шаг:**
- Когда Telegram API доступен с VPS — refresh карточки Бабич из HR или повторный вызов notify.
...
**Сделано:**
- После решения в клиентской зоне (`apply_zone_decision`) вызывается `_sync_zone_decision_outbound` — обновление задачи Bitrix, как при decide-ссылке (отказ / подумать / встреча).
- UX: после успешного decide редирект на список кандидатов с баннером «Решение сохранено» (больше не 404 на карточке после отказа).
- Ручной догон на проде: задача Bitrix #340 для Бабич синхронизирована (`sync_ok True`).

**Файлы:** `v2/backend/app/services/client_zone.py`, `v2/frontend/app/c/[token]/page.tsx`, `v2/frontend/app/c/[token]/[candidateId]/page.tsx`

**Git:** незакоммиченные изменения

**Риски:** deploy нужен для fix на проде; догон Бабич уже выполнен вручную.

**Следующий шаг:**
- Задеплоить sidecar и проверить отказ из клиентской зоны на новом кандидате.
...
**Сделано:**
- Сброшены пароли всех 7 пользователей в БД sidecar (`hr-v2-side-api`).
- Отозваны refresh-сессии (142 токена).
- Обновлён `AUTH_BOOTSTRAP_PASSWORD` в `/opt/hr_ai_agent/v2/.env.sidecar` под `owner@hr.local`.
- Новые пароли переданы владельцу в чате (не коммитить).

**Данные / конфиг:** `/opt/hr_ai_agent/v2/.env.sidecar` на сервере Timeweb

**Git:** без изменений в репо

**Следующий шаг:**
- Владелец сохраняет пароли локально; при желании удалить тестовых users

---

## 2026-08-16 — ЭФИР: сценарий видео + аватар (план)

**Тип:** `decision`

**Сделано:**
- План `IMPLEMENTATION_PLAN_EFIR.md`: фича А (сценарий+суфлёр) → QUEUE; фича Б (аватар) → BACKLOG B0–B2; бонус покрытие сценария → B-EFIR-002.
- Оценка готовых решений: А принять (оценка 2–3 дн); Б — модуль, SpeechKit сейчас только STT ≠ TTS.
- В КАСКАД task_map: `video_interview_script=top`, `avatar_pitch_compress=fast`.
- Кодовое слово: **`ЭФИР`**.

**Файлы:** `v2/docs/IMPLEMENTATION_PLAN_EFIR.md`, `QUEUE.md`, `BACKLOG.md`, `IMPLEMENTATION_PLAN_KASKAD.md`

**Git:** незакоммичено

**Следующий шаг:**
- Реализация А по **`ЭФИР A`**; Б — после выбора провайдера **`ЭФИР B0`**

---

## 2026-08-15 — Бэклог: АССИСТЕНТ (план)

**Тип:** `docs` / `decision`

**Сделано:**
- Пофазовый план ИИ-ассистента A1→A2→A3 (без кода).
- Запись **B-ASSISTENT-001** в BACKLOG (planned).
- Зафиксированы риски: нет HELP.md, labels только на фронте, не кормить весь ARCHITECTURE в prompt, не в `/c/`, A3 сначала rule-based.

**Файлы:** `v2/docs/IMPLEMENTATION_PLAN_ASSISTENT.md`, `v2/docs/BACKLOG.md`

**Git:** незакоммичено

**Следующий шаг:**
- Старт кода только по команде `АССИСТЕНТ A1`.

---

## 2026-08-15 — Роль при решении в зоне + Telegram

**Тип:** `feature`

**Сделано:**
- Во всех клиентских зонах обязательный выбор роли: Руководитель подразделения / Директор / Собственник (ключи `unit_head` | `director` | `owner`).
- Роль пишется в payload и в подпись комментария; без роли API не принимает решение.
- После решения: в чате обновляется статус на карточке, кнопка «Смотреть» остаётся, reply — статус (+ дата/время для встречи) и роль.
- Выложено на hr-toolbox.ru (`api` + `web`); health OK.

**Файлы:** `client_zone.py`, `routes/client_zone.py`, `ClientZoneDecideForm.tsx`, `clientZone.ts`, `ops.py`, `card_html.py`, `inbound.py`, `client_apply.py`, `globals.css`

**Git:** незакоммичено

**Следующий шаг:**
- Проверить решение в зоне: роль в форме + reply в Telegram.

---

## 2026-08-15 — Новые иконки аватарок вакансий

**Тип:** `feature`

**Сделано:**
- В выбор аватарки добавлены 6 вариантов: карандаш, монитор, ящик/груз, !, три человека, ?.
- Только картинка в пикере; подпись — во всплывающей подсказке.

**Файлы:** `vacancy_avatar.py`, `VacancyAvatar.tsx`

**Git:** незакоммичено

**Следующий шаг:**
- В настройках вакансии выбрать новую иконку.

---

## 2026-08-15 — Аватар в клиентской зоне + фото Бучневой

**Тип:** `feature` / `ops`

**Сделано:**
- Клиентская зона отдаёт `photo_url` и пол; аватар в списке и в шапке карточки.
- Бучнева Мария: фото из PDF на Диске → S3.
- Выложено на hr-toolbox.ru (`api` + `web`).

**Файлы:** `client_zone.py`, `clientZone.ts`, `app/c/…`, `globals.css`

**Git:** незакоммичено

**Следующий шаг:**
- Обновить `/c/…` и карточку Бучневой.

---

## 2026-08-14 — Фото всем кандидатам Lamoda

**Тип:** `ops`

**Сделано:**
- Вакансия 15: фото из PDF на Диске проставлены Манаенковой, Еделкиной, Васильевой Н.; Бабич уже была.
- У Васильевой Haar лицо не нашёл — взяли встроенное фото 375×500.
- Случайно записанное фото с чужой карточки «Васильева Яна» снято.

**Данные / конфиг:** prod vacancy 15, S3 `photos/{id}.jpg`

**Следующий шаг:**
- Обновить список вакансии Lamoda.

---

## 2026-08-14 — Клиентская зона: оценка ИИ ниже конспекта

**Тип:** `fix`

**Сделано:**
- Кнопка и текст «Оценка ИИ» идут после кнопки и текста «Конспект собеседования», а не между ними.

**Файлы:** `app/c/[token]/[candidateId]/page.tsx`

**Git:** незакоммичено

**Следующий шаг:**
- Обновить карточку в клиентской зоне.

---

## 2026-08-14 — Фото кандидата в списке вакансии

**Тип:** `fix`

**Сделано:**
- Список кандидатов вакансии отдаёт `photo_url` и пол — раньше в ответе их не было, поэтому в строке была заглушка.

**Файлы:** `routes/vacancies.py`

**Git:** незакоммичено

**Следующий шаг:**
- Обновить список кандидатов вакансии на hr-toolbox.ru.

---

## 2026-08-14 — Клиентская зона: конспект и оценка ИИ кнопками

**Тип:** `feature`

**Сделано:**
- Кнопка выжимки: «Конспект собеседования» / «Скрыть конспект собеседования».
- «Оценка ИИ» — такая же кнопка в материалах, текст свёрнут до нажатия.
- В списке метка «конспект» вместо «выжимка».

**Файлы:** `app/c/[token]/[candidateId]/page.tsx`, `app/c/[token]/page.tsx`

**Git:** незакоммичено

**Следующий шаг:**
- Проверить карточку кандидата на hr-toolbox.ru.

---

## 2026-08-14 — Деплой: OpenCV 4 + клиентская зона + Telegram IPv4

**Тип:** `deploy`

**Сделано:**
- Rsync на `201.34.137.208`, пересборка sidecar `api` / `worker` / `web`.
- OpenCV в worker: **4.14.0**, Haar на месте. Health OK.
- Вместе ушли: клиентская зона (компания · подразделение, выжимка), Telegram IPv4.
- Фото Бабич в карточке сохранилось.

**Git:** незакоммичено (после `08f15b7`)

**Следующий шаг:**
- Обновить hr-toolbox.ru и проверить карточку Бабич / клиентскую зону.

---

## 2026-08-14 — Тест фото Бабич + OpenCV 5 ломает Haar

**Тип:** `fix` / `ops`

**Сделано:**
- Прогон фото по PDF Бабич на сервере: файл в «Резюме» вакансии Lamoda.
- На сервере стоял OpenCV 5: нет `CascadeClassifier` и Haar XML — штатное извлечение лица падает.
- Для теста загружено встроенное фото 333×500 (фильтр пропорций 0.7 его чуть отсекает: 0.666) в S3 `photos/{id}.jpg`.
- В карточке выставлены `photo_url` и `resume_link`.
- В `requirements.txt` OpenCV зафиксирован на `>=4.10,<5` (ещё не задеплоено).

**Файлы:** `v2/backend/requirements.txt`

**Данные / конфиг:** prod кандидат `52801f72-…`, бакет `my-hr-audio-bucket`

**Следующий шаг:**
- Обновить карточку Бабич в UI; для авто-фото нужен деплой с OpenCV 4.

---

## 2026-08-14 — Сервер: остановлена расшифровка Бабич

**Тип:** `ops`

**Сделано:**
- Задача `candidate_interview_process` `4831e403-…` (Бабич Дарья) отменена: скачивание 145 МБ с Диска зависло (~30 КБ/с).
- Воркер перезапущен, временный файл удалён. Повторно задача не стартовала.

**Данные / конфиг:** prod `hr-v2-side-*`, кандидат `52801f72-…`

**Следующий шаг:**
- После сжатия файла — снова «Расшифровать и оценить».

---

## 2026-08-14 — Клиентская зона: компания + выжимка

**Тип:** `feature`

**Сделано:**
- В списке и карточке кандидата видно «компания · подразделение», не только отдел.
- Шапка зоны — название компании; у подразделения оно дублируется под заголовком.
- Кнопка «Краткий текст собеседования» стоит в блоке «Материалы»; по нажатию раскрывается текст на этой же странице.
- В списке метки материалов: резюме · запись · выжимка.

**Файлы:** `client_zone.py`, `lib/clientZone.ts`, `app/c/[token]/page.tsx`, `app/c/[token]/[candidateId]/page.tsx`, `globals.css`

**Git:** незакоммичено

**Следующий шаг:**
- Открыть `/c/{token}` и карточку кандидата на телефоне; проверить компанию и кнопку выжимки.

---

## 2026-08-14 — Fix: Telegram из Docker (IPv4) + дубль ошибки в UI

**Тип:** `fix`

**Сделано:**
- `telegram_provider.py`: принудительно IPv4 для requests (Docker без IPv6-маршрута → «Network is unreachable» к api.telegram.org).
- `CandidateEditor`: убран повтор ошибок Telegram в блоке «Заказчик» (errors уже в `message`).
- Hotfix задеплоен на hr-toolbox.ru (api/worker).

**Git:** незакоммичено (после `08f15b7`)

---

## 2026-08-18 — Кадровый резерв (MVP)

**Тип:** `feature`

**Сделано:**
- Флаг `talent_reserve` в `candidate.payload` + дата/автор при добавлении.
- API: `GET /candidates?preset=talent_reserve`, PATCH `talent_reserve`.
- Страница `/talent-reserve` (меню «Ещё»), кнопки в шапке карточки кандидата.

**Файлы:** `candidate_query.py`, `candidate_write.py`, `candidate_fields.py`, `jsonb_defaults.py`, `schemas.py`, `routes/candidates.py`, `CandidateEditor.tsx`, `RecruitingShell.tsx`, `app/talent-reserve/page.tsx`, `globals.css`, `api.ts`

**Git:** незакоммичено на `feature/v2`

**Риски:** один человек на двух вакансиях может быть в резерве дважды; миграция в `talent_pool_entries` — при ЯКОРЬ PR3.

**Следующий шаг:**
- Проверить на hr-toolbox.ru: отметить кандидата → зона «Кадровый резерв».

---

## 2026-08-18 — Ветка реализации: feature/kaskad-yakor-assistent-efir

**Тип:** `docs` / `decision`

**Сделано:**
- Переименована ветка `docs/plans-kaskad-yakor-assistent-efir` → `feature/kaskad-yakor-assistent-efir`.
- В `BACKLOG.md` зафиксировано: сюда — планы **и вся реализация** по **КАСКАД**, **ЯКОРЬ**, **АССИСТЕНТ**, **ЭФИР**; в `feature/v2` — макеты, демо, прочие фичи до merge.
- Старая remote-ветка `docs/plans-…` удалена.

**Файлы:** `v2/docs/BACKLOG.md`

**Git:** branch `feature/kaskad-yakor-assistent-efir`, commits `79061f4`, `6b080a7`, push origin

**Следующий шаг:**
- При старте по кодовому слову — checkout `feature/kaskad-yakor-assistent-efir`.
...
## 2026-08-14 — КАСКАД: слайсинг утверждён, fallback-матрица, оценки

**Тип:** `decision`

**Сделано:**
- Дизайн КАСКАД утверждён; порядок: P1 сейчас → ЯКОРЬ PR1+PR2 → КАСКАД P2 → P3+P4 после пилота.
- В план добавлены: слайсинг с оценками по фазам; матрица fallback на 4 случая; гейт P4 = выборка baseline, не «уже доказанная экономия».
- P1 без `task=` на call sites не принимать.

**Файлы:** `v2/docs/IMPLEMENTATION_PLAN_KASKAD.md`, `v2/docs/BACKLOG.md`

**Git:** незакоммичено

**Следующий шаг:**
- Старт кода по команде **`КАСКАД P1`**
...
**Сделано:**
- Оформлен итоговый план (архитектор v2): persons, D1–D3, talent pool T, OpenAPI, alembic h1–h4, PR1/PR2/PR3.
- Кодовое слово запуска: **`ЯКОРЬ`** (PR1/PR2/PR3/smoke) — без него план не трогаем.
- Runbook гонки persons без UNIQUE.

**Файлы:** `v2/docs/IMPLEMENTATION_PLAN_YAKOR.md`, `.cursor/runbooks/yakor-person-race.md`

**Git:** незакоммичено

**Следующий шаг:**
- `ЯКОРЬ smoke` → зелёный SMOKE_TEST §1–24 → `ЯКОРЬ PR1`.

---

## 2026-08-14 — UI: Шаблоны и Клиентская зона → RecruitingShell

**Тип:** `fix`

**Сделано:**
- `/templates` и `/client-zone` переведены с `AppShell` на `RecruitingShell` (левая панель как у вакансий/кандидатов).
- Контент в `rec-card`, таблица шаблонов — `rec-table`.
- Подсказка под заголовком (`rec-page-lead`), иконка «Клиентская зона» в меню — Share2.

**Файлы:** `client-zone/page.tsx`, `templates/page.tsx`, `RecruitingShell.tsx`, `globals.css`

**Git:** незакоммичено

**Следующий шаг:**
- Проверить в браузере оба раздела.
...
**Сделано:**
- Документы: после генерации/материалов обновляется preview (`savedDocs`).
- Контрольное слово: проверка при оценке резюме + блок в анкете кандидата.
- Полоска этапов берёт схему вакансии (названия и включённые этапы).
- «Следующий шаг»: оценка → вкладка ИИ; подтверждение встречи → скролл к блоку.
- Повторная фиксация этапа с комментарием пишет в историю.
- Оффер Word: устойчивее подстановка в пользовательский .docx.
- Настройки: убраны технические пути к `app_settings.json`.

**Файлы:** `candidate_resume_eval.py`, `candidate_write.py`, `common.py`, `schemas.py`, `offer_docx.py`, `DocumentsEditor.tsx`, `StageProgress.tsx`, `CandidateEditor.tsx`, `nextAction.ts`, `labels.ts`, `api.ts`, settings pages, `FIX_QUEUE.md`

**Git:** незакоммичено

**Следующий шаг:**
- P3: Zoom / приглашения; затем проверка на стенде.
...
**Сделано:**
- Дубликат названия вакансии проверяется только внутри org (не по всей БД).
- «Оценить резюме ИИ» — отдельный режим без формирования опросника (`skip_questionnaire`).
- Вкладка «Интервью»: опросник не размонтируется при смене вкладки (`hidden`).

**Файлы:** `vacancy_write.py`, `candidate_resume_eval.py`, `candidates.py`, `tasks.py`, `schemas.py`, `QuestionnairePanel.tsx`, `CandidateEditor.tsx`, `FIX_QUEUE.md`

**Git:** незакоммичено

**Следующий шаг:**
- P2: документы вакансии, контрольное слово, схема этапов.
...
**Причина:** старый ARQ worker с `HTTPS_PROXY` Cursor продолжал брать задачи → 403 Tunnel к routerai.ru.

**Сделано:**
- `chat_json`: `trust_env=False` + явный `proxies={http/https: None}`.
- Worker `on_startup`: сброс всех proxy-переменных, `NO_PROXY=*`.
- Скрипт `scripts/run_worker_local.sh` — чистый старт без прокси.
- Убит дубль старого worker; один чистый процесс.

**Файлы:** `ai_json.py`, `workers/settings.py`, `scripts/run_worker_local.sh`

**Git:** незакоммичено

**Следующий шаг:**
- Повторить оценку резюме в UI.

---

## 2026-08-13 — Добавление кандидатов: не зависать на ИИ

**Тип:** `fix`

**Причина:** при «Сразу оценить» HTTP ждал RouterAI 1–2 мин → таймаут/обрыв; карточка оставалась «Новый кандидат» без оценки.

**Сделано:**
- Добавление по ссылке/файлу больше не вызывает ИИ синхронно.
- Оценка уходит в фоновую задачу `candidate_evaluate_resume` (нужен worker).
- Имя из имени файла PDF, если есть.
- Локально запущен ARQ worker.

**Файлы:** `candidate_resume_eval.py`, `vacancies.py`, `schemas.py`, `AddCandidateForm.tsx`

**Git:** незакоммичено

**Следующий шаг:**
- Проверить добавление файла/ссылки на localhost.

---

## 2026-08-13 — Кнопка «Создать ссылку» клиентской зоны

**Тип:** `fix`

**Сделано:**
- После создания ссылки путь сразу берётся из ответа API (не ждём только перезагрузку списка).
- Пути подразделений хранятся в состоянии; ошибка показывается рядом с блоком зоны.
- Перезапущен зависший локальный API на `:8000` (reload застрял).

**Файлы:** `CompanyEditor.tsx`, `ClientZoneLink.tsx`

**Git:** незакоммичено

**Следующий шаг:**
- Пользователь проверяет «Создать ссылку» на localhost.

---

## 2026-08-13 — Клиентская зона: отдельная ссылка на подразделение

**Тип:** `fix`

**Сделано:**
- Токен зоны живёт на конкретной записи клиента (компания или подразделение), а не только на корне.
- Публичная ссылка `/c/…` показывает только кандидатов этой зоны, без чужих отделов.
- В настройках компании и на хабе «Клиентская зона» — отдельная ссылка на каждое подразделение.
- Отправка заказчику создаёт/отдаёт ссылку подразделения вакансии.

**Файлы:** `tenancy.py`, `client_zone.py`, `clients.py`, `registry.py`, `clients_write.py`, `schemas.py`, `CompanyEditor.tsx`, `client-zone/page.tsx`

**Git:** незакоммичено

**Риски/регрессии:** старая ссылка компании больше не показывает кандидатов подразделений — для отделов нужно создать свои ссылки.

**Следующий шаг:**
- Выложить на сервер; создать ссылки для существующих подразделений.

---

## 2026-08-13 — Очередь смоук-теста: клиентская зона, сессия, PDF-оценка

**Тип:** `fix` / `ux`

**Сделано:**
- Очередь правок из Excel: `v2/testing/FIX_QUEUE.md` (15 пунктов по приоритету).
- Клиентская зона: компонент `ClientZoneLink` (полный URL, копирование, инструкция); хаб `/client-zone` со списком компаний; постоянная ссылка на вкладке «Заказчик»; понятнее пустое состояние `/c/[token]`; текст при отправке в веб-канал.
- Сессия: `authMe()` пробует refresh при 401 — после паузы не выкидывает сразу на вход.
- PDF: «Сразу оценить по резюме» включено по умолчанию при загрузке файла.

**Файлы:** `ClientZoneLink.tsx`, `client-zone/page.tsx`, `CompanyEditor.tsx`, `CandidateEditor.tsx`, `c/[token]/page.tsx`, `AddCandidateForm.tsx`, `lib/api.ts`, `globals.css`, `messaging/providers/registry.py`, `FIX_QUEUE.md`

**Git:** незакоммичено

**Риски/регрессии:** ссылка клиентской зоны видна только при включённом канале `web` в настройках уведомлений.

**Следующий шаг:**
- П.3 очереди: ложный конфликт дубликата вакансии; деплой правок на сервер.

---

## 2026-08-13 — SpeechKit ключи + ffmpeg на сервере

**Тип:** `ops` / `fix`

**Причина:** расшифровка интервью падала: нет `YANDEX_*` SpeechKit в `.env.sidecar`; в образе не было `ffmpeg`.

**Сделано:**
- В `.env.sidecar` на сервере добавлены `YANDEX_API_KEY`, `YANDEX_BUCKET_NAME`, `YANDEX_ACCESS_KEY_ID`, `YANDEX_SECRET_ACCESS_KEY` из локального корневого `.env`.
- В `backend/Dockerfile` установлен `ffmpeg`; api/worker пересобраны.
- Проверка: `keys_ok True`, `ffmpeg /usr/bin/ffmpeg`.

**Файлы:** `v2/backend/Dockerfile`, `v2/.env.sidecar.example`
...
**Следующий шаг:**
- Повторить «Расшифровать и оценить» у Манаенковой.

---

## 2026-08-13 — Смоук-авто + Bitrix + том data

**Тип:** `ops` / `docs`

**Сделано:**
- Отчёт авто-смоука: `v2/testing/results/2026-08-13-smoke-auto.md`.
- В `docker-compose.sidecar.yml` api/worker: том `../data:/app/data` + `LEGACY_DATA_DIR=/app/data` (чтобы `app_settings` / OAuth не слетали при recreate).
- На хосте `/opt/hr_ai_agent/data/app_settings.json`: Bitrix включён, `public_api_base=https://hr-toolbox.ru`, каналы `bitrix`+`web`, HH search on.
- api/worker пересозданы; проверка: `bitrix_enabled=True`, webhook есть.

**Файлы:** `docker-compose.sidecar.yml`, `v2/testing/results/2026-08-13-smoke-auto.md`

**Данные / конфиг:** `/opt/hr_ai_agent/data/app_settings.json` на сервере

**Git:** незакоммичено

**Следующий шаг:**
- Ручная проверка кликов по боковому меню (в авто части Link не меняли URL); Zoom OAuth под owner.

---

## 2026-08-12 — Яндекс.Диск: токен на сервере

**Тип:** `ops`

**Причина:** на сервере не было OAuth-токена Я.Диска → ошибка при оценке кандидата с файлами на Диске.

**Сделано:**
- `YANDEX_DISK_OAUTH_TOKEN` добавлен в `.env.sidecar` (из локального `data/yandex_disk_oauth.json`).
- Файл `yandex_disk_oauth.json` скопирован в `/opt/hr_ai_agent/data/` и в контейнер.
- В `app_settings.json` на сервере: `yandex_disk_root`, `yandex_disk_inbox`, `yandex_disk_client_id`.
- Перезапущены api + worker; `disk_status().connected = True`.

**Данные / конфиг:** `.env.sidecar`, `data/yandex_disk_oauth.json` на сервере (не в git)

**Git:** только `.env.sidecar.example`

**Следующий шаг:**
- При отзыве токена — переподключить Диск в Настройках или обновить токен в `.env.sidecar`.

---

## 2026-08-12 — HH: токены на сервере для поиска резюме

**Тип:** `ops`

**Причина:** на сервере в `.env.sidecar` не было `HH_*` → ошибка «Не задан HH_ACCESS_TOKEN».

**Сделано:**
- Скопированы `HH_CLIENT_ID`, `HH_CLIENT_SECRET`, `HH_ACCESS_TOKEN`, `HH_REFRESH_TOKEN`, `HH_USER_AGENT` из локального `.env` в `/opt/hr_ai_agent/v2/.env.sidecar`.
- Перезапущены api + worker; `get_hh_access_token()` возвращает токен.
- В `.env.sidecar.example` добавлен блок HH с инструкцией.
...
**Следующий шаг:**
- При истечении токена — `scripts/hh_oauth.py` или обновить refresh в `.env.sidecar`.

---

## 2026-08-12 — Bitrix включён на сервере для Pilot Recruiter

**Тип:** `ops`

**Сделано:**
- В `data/app_settings.json` на сервере: `bitrix.enabled=true`, webhook Bitrix24, `public_api_base=https://hr-toolbox.ru`, ответственный по умолчанию `32`.
- Каналы уведомлений заказчику: `bitrix` + `web` (уже были).
- У `pilot@demo.ru` уже был `bitrix_responsible_id=32` — задачи в Bitrix идут на этого ответственного.

**Данные / конфиг:** `data/app_settings.json` на сервере (не в git)

**Git:** не трогали

**Следующий шаг:**
- Проверить «Отправить заказчику» у Pilot Recruiter → задача в Bitrix24.

---

## 2026-08-12 — Fix: HTTP 500 при добавлении кандидата по ссылке

**Тип:** `fix`

**Причина:** в `candidate_resume_eval.py` вызывался `fetch_resume_text_from_url` без импорта → `NameError` → HTTP 500 на `POST …/candidates/bulk-links`.

**Сделано:**
- Добавлен импорт из `app.services.pdf_extract`.

**Файлы:** `candidate_resume_eval.py`

**Деплой:** api + worker пересобраны на hr-toolbox.ru

**Git:** незакоммичено

---

## 2026-08-12 — Кузовов: перенос в пустую организацию

**Тип:** `ops`

**Сделано:**
- Пользователь `kuzovov@test.hr-toolbox.ru` перенесён из «Default Organization» в новую org «Тест Кузовов» (`kuzovov-test`).
- В новой org: 0 клиентов, 0 вакансий — ваши данные не видны.
- Роль без изменений: `hr_recruiter`. Логин/пароль те же.

**Git:** не трогали

---

## 2026-08-12 — Тестовый пользователь Кузовов на сервере

**Тип:** `ops`

**Сделано:**
- Создан пользователь `kuzovov@test.hr-toolbox.ru`, роль `hr_recruiter`, ФИО «Кузовов Тест».
- Пароль сгенерирован и передан пользователю в чат (не в git).

**Git:** не трогали

**Следующий шаг:**
- Пользователь меняет пароль после первого входа (если добавим смену пароля в UI).

---

## 2026-08-12 — Плейсхолдеры аватарок кандидатов (муж/жен)

**Тип:** `ui`

**Сделано:**
- Вместо SVG-силуэтов и букв в `CandidateAvatar` — PNG-плейсхолдеры по полу: `/candidate-placeholder-male.png`, `/candidate-placeholder-female.png`.
- Пол: из поля `gender` или эвристика по ФИО; если не определён — мужской плейсхолдер.
- CSS: `object-fit: cover`, `object-position: center 18%` для круглого кропа головы.

**Файлы:** `CandidateAvatar.tsx`, `globals.css`, `public/candidate-placeholder-*.png`

**Деплой:** sidecar web пересобран на hr-toolbox.ru

**Git:** незакоммичено

**Следующий шаг:**
- При необходии закоммитить и подтянуть на сервер через git.

---

## 2026-08-12 — Чек-лист смоук-теста перед передачей тестовому пользователю

**Тип:** `docs`

**Сделано:**
- Собран полный список функций: все маршруты фронтенда (включая вкладки вакансии и кандидата, три режима аналитики, все разделы настроек) и все модули API бэкенда.
- Проверено, что реально включено на сервере: ИИ (RouterAI) работает; Telegram, HH, Яндекс.Диск, SpeechKit, Zoom, Google Calendar, Bitrix — не настроены (`APP_ENV=pilot`, `MESSAGING_*=false`, ключей нет). `client_notify.channels = [bitrix, web]`.
- Создан чек-лист на 180 пунктов в 25 разделах, порядок — как рабочий день: вход → вакансия → кандидат → заказчик → аналитика → настройки → устойчивость.
- В шапке чек-листа зафиксировано, какие интеграции выключены, чтобы это не считали багами; для них проверяется только понятное сообщение вместо падения.
- Заведена папка для заполненных прогонов и скриншотов.

**Файлы:** `v2/testing/SMOKE_TEST.md`, `v2/testing/results/.gitkeep`

**Данные на сервере на момент проверки:** 8 компаний, 5 вакансий, 55 кандидатов.

**Git:** незакоммичено

**Следующий шаг:**
- Пройти чек-лист, положить заполненную копию в `v2/testing/results/` и разобрать пункты `НЕ ОК`.

---

## 2026-08-12 — Вход → главная, тема «дуб» по умолчанию, возврат текстуры

**Тип:** `ui`

**Сделано:**
- После входа всегда открывается главная (`/`), а не последний раздел. Из `middleware.ts` убран параметр `next`, страница входа его больше не читает.
- Тема `oak` — по умолчанию: `DEFAULTS` в `UiPrefsProvider`, `data-theme="oak"` в `layout.tsx`.
- Возвращена текстура дерева: слои (SVG-зерно + прожилки + градиент доски) перенесены в фон `body` вместо псевдоэлемента; `.rec-app`, `.home-v3`, `.login-page` сделаны прозрачными, `.rec-main` — лёгкая вуаль 34%. Раньше сплошная заливка этих блоков закрывала текстуру.

**Файлы:** `login/page.tsx`, `middleware.ts`, `UiPrefsProvider.tsx`, `layout.tsx`, `globals.css`

**Проверено:** скриншоты главной и входа — текстура видна; `/dashboard` без сессии → 307 на `/login`; `tsc --noEmit` чисто.

**Git:** незакоммичено

---

## 2026-08-12 — Fix: зависание «Проверка входа» при входе в Поиск сотрудников

**Тип:** `fix`

**Причина (найдена через отладку в браузере):**
- Без сессии server component страницы получал 401 от API. `apiFetch` вызывал `redirect("/login")`, но страницы оборачивают `apiGet` в `try/catch` и глотали служебную ошибку `NEXT_REDIRECT`.
- Тело ответа 401 оставалось непрочитанным → в потоке серверного рендера висело незакрытое обещание → React не гидрировал страницу → «Проверка входа…» навсегда.

**Сделано:**
- Новый `middleware.ts`: без cookie `hr_access` / `hr_refresh` защищённые страницы уходят на `/login?next=…` **до** серверного рендера. Публичные: `/`, `/login`, `/c/*`, `/i/*`, `/design-preview*`.
- `lib/api.ts`: на сервере при 401 больше нет `redirect()`; тело ответа дочитывается, бросается `AuthRequiredError`.
- Откачены прошлые неудачные правки `AuthGate.tsx` и `JobsLive.tsx` (причина была не в них).

**Файлы:** `v2/frontend/middleware.ts` (новый), `v2/frontend/lib/api.ts`

**Проверено:** без cookie `/candidates` → 307 на `/login`; со старой cookie — страница входа после проверки. `tsc --noEmit` чисто.

**Git:** незакоммичено

**Следующий шаг:**
- Войти локально и проверить тему «Светлый дуб»; затем деплой на hr-toolbox.ru.

---

## 2026-08-12 — Тема «Светлый дуб» для всего интерфейса

**Тип:** `ui`

**Сделано:**
- Новая тема `oak` в настройках внешнего вида (рядом с светлой, тёмной, earth, citrus, sky).
- Древесный фон и бежевые тона на сайдбар, рабочую зону и главную; переменные `--rec-frame` / `--rec-main` для всех тем.
- Главная и rec-shell читают цвета из темы, не из захардкоженного серого.

**Файлы:** `UiPrefsProvider.tsx`, `AppearanceSettings.tsx`, `settings/page.tsx`, `globals.css`, `page.tsx`

**Git:** незакоммичено

---

## 2026-08-12 — Главная: фон светлого дуба (промежуточный шаг)

**Тип:** `ui`

**Сделано:**
- Первый вариант дубового фона был только на главной (`home-v3-wood`); затем заменён общей темой `oak` (запись выше).

**Файлы:** `page.tsx`, `globals.css`

**Git:** незакоммичено

---

## 2026-08-12 — Лого в сайдбаре → главная (хаб модулей)

**Тип:** `ui`

**Сделано:**
- Клик по логотипу и «HR-помогатор» в левой панели ведёт на `/` (хаб с «Поиск сотрудников» и заглушками).

**Файлы:** `RecruitingShell.tsx`

**Git:** незакоммичено

**Деплой:** hr-toolbox.ru, web пересобран, health OK
